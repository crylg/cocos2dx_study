...
static void transform(unsigned long *state, const byte block[64]) 
{
...
void MD5::Update(const void* data, ulong length)
{
...
void MD5::ToString(char (&output)[33]) const
{
...
std::string MD5Encrypt(const void* data, ulong length)
{
	MD5 md5;
	md5.Begin();
	md5.Update((const byte*)data, length);
	md5.Final();
	char output[33];
	md5.ToString(output);
